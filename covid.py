...
required_columns=["Province_State", "Country_Region", "Last_Update", "Confirmed", "Incidence_Rate","Deaths", "Recovered"]
...
def reshape_schema(df: pd.DataFrame) -> pd.DataFrame:
    df=df.rename(columns=lambda c: New_Name.get(c, c))
    for col in superset_columns:
        if col not in df.columns:
            df[col]=pd.NA
    return df[superset_columns]

def validate(df: pd.DataFrame, fname: str):
...
for day in range(days):
    single_date=start_date + timedelta(day) # timedelta adds number in terms of days
